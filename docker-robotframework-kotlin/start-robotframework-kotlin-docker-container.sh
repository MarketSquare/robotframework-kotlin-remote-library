...
if [ "$1" == "Run-Docker-Container" ]; then
  ...
  source ./.env
  echo
  echo "------------------------------------[[[[ Robot Framework Docker Container - Kotlin Native Application Tests ]]]]------------------------------------"
  echo
  echo "This Docker Container will run various Robot Framework tests on a simple Kotlin Native Application. This run started on $TIMESTAMP."
  echo
  docker-compose -f docker-compose.yml down
  docker-compose -f docker-compose.yml rm -f
  docker-compose -f docker-compose.yml build
  docker-compose run docker-robot-framework-kotlin-test-runner commands-running-inside-a-docker-container.sh
